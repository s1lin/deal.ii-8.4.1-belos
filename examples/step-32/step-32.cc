...
namespace Step32
{
  ...
  namespace LinearSolvers
  {
    ...
    public:
                  ...
                  const TrilinosWrappers::MPI::BlockVector &src) const
      {
        ...
        if (do_solve_A == true)
          {
            //std::cout<<"IN:tol = "<<utmp.l2_norm()*1e-2<<std::endl;
            SolverControl solver_control(5000, utmp.l2_norm()*1e-3);
            TrilinosWrappers::SolverCG solver(solver_control);
            double actTol = solver.solve(stokes_matrix->block(0,0), dst.block(0), utmp,
                         a_preconditioner);
            //std::cout<<"OT:Tol="<<actTol<<std::endl;
          }
        else
  ...
  void BoussinesqFlowProblem<dim>::solve ()
  {
    ...
    {
      pcout << "   Solving Stokes system... " << std::flush<<std::endl;
